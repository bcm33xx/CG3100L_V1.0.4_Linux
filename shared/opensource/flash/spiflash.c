...
#if defined(CONFIG_BCM93380)
static int (*rpc_read_buf)(void *, unsigned int, unsigned char *, int) = NULL; 
...
{
    rpc_handle = arg;
    rpc_read_buf = (int (*)(void *, unsigned int, unsigned char *, int))p;
...
static int spi_flash_sector_erase_int(unsigned short sector)
{
    ...
    if (my_spi_write(buf, sizeof(buf)) == SPI_STATUS_OK) {
    ...
    unsigned char *buffer, int numbytes)
{
    ...
    if(rpc_handle)
    {
        ...
        addr -= 0x00800000;
        /* printk("spi_flash_read_buf: sector=%d addr=%08x offset=%d\n", sector, addr, offset); */
        rpc_read_buf(rpc_handle, addr, buffer, numbytes);
    }
    else
    {
        if(spi_dual_flash)
    ...
    while (numbytes > 0) {
        ...
        my_spi_read(buf, prependCnt, maxread);
        while (spi_flash_status() != STATUS_READY);
...
static int at_spi_flash_ub(unsigned short sector)
{
    ...
    buf[2] = (unsigned char)((addr & 0x0000ff00) >> 8);
    buf[3] = (unsigned char)(addr & 0x000000ff);
    if (my_spi_read(buf, 4, 1) == SPI_STATUS_OK) {
    ...
    if (my_spi_write(buf, 1) == SPI_STATUS_OK) {
        ...
        buf[2] = (unsigned char)((addr & 0x0000ff00) >> 8);
        buf[3] = (unsigned char)(addr & 0x000000ff);
        if (my_spi_write(buf, 4) == SPI_STATUS_OK)
            while (spi_flash_status() != STATUS_READY);
    }
    ...
    switch (flashFamily) {
        ...
        case FLASH_SPAN:
            while (nbytes > 0) {
                ...
                my_spi_write(wbuf, maxwrite+CMD_LEN_4);
                while (spi_flash_status() != STATUS_READY);
                pbuf += maxwrite;
                nbytes -= maxwrite;
                dst += maxwrite;
...
static unsigned short spi_flash_get_device_id(unsigned short sector)
{
    ...
    for (i = 0; i < MAX_RETRY; i++) {
        ...
        buf[3] = 0;
        prependcnt = 4;
        my_spi_read(buf, prependcnt, sizeof(unsigned short));
