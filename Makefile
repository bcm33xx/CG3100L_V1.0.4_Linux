...
all_postcheck1: sanity_check \
     create_install kernelbuild modbuild kernelbuildlite fapbuild switchsdkbuild\
...
$(KERNEL_DIR)/vmlinux:
	...
	cp -f $(KERNEL_DIR)/arch/mips/defconfig.3380 $(KERNEL_DIR)/.config; \
...
fapbuild:
	@echo "******************** FAP Build ********************"; \
	$(BRCMDRIVERS_DIR)/broadcom/char/fap/impl1/getFapModSizes.sh  $(BRCMDRIVERS_DIR)/broadcom/char/fap/impl1/bcmfap.ko $(SHARED_DIR) $(CROSS_COMPILE) $(KERNEL_DEBUG);
	cd $(KERNEL_DIR); $(MAKE)
else
fapbuild:
endif
...
modules: sanity_check create_install modbuild kernelbuildlite fapbuild switchsdkbuild hosttools buildimage
...
ifeq ($(strip $(BRCM_KERNEL_ROOTFS)),jffs2)
	...
	cat cferom.tmp rootfs128kb.img > $(FLASH_NAND_RAW_FS_IMAGE_NAME_128); \
	$(HOSTTOOLS_DIR)/addvtoken $(FLASH_NAND_RAW_FS_IMAGE_NAME_128) $(FLASH_NAND_RAW_FS_IMAGE_NAME_128).w; \
	$(HOSTTOOLS_DIR)/addvtoken rootfs128kb.img $(FLASH_NAND_FS_IMAGE_NAME_128).w; \
	rm $(FLASH_NAND_RAW_FS_IMAGE_NAME_16) $(FLASH_NAND_RAW_FS_IMAGE_NAME_128); \
...
clean: target_clean bcmdrivers_clean userspace_clean kernel_clean \
       hosttools_clean voice_clean xchange_clean switchsdk_clean
